...
namespace Lista2
{
    public class Program
    {
        ...
        public void GenerateGraphZ3(string graphName, HashGenerator hashGenerator, Dictionary<int, List<double>> dictionary)
        {
            new[] { 2, 3, 10, 20, 100, 400 }.ToList().ForEach(k =>
                  {
                      var result = hashGenerator.GetForK(k, dictionary);
        ...
        public void GenerateGraphZ5(string graphName, HashGenerator hashGenerator,
            Dictionary<int, List<double>> dictionary, int k)
        {
            new[] { 0.05, 0.01, 0.005 }.ToList().ForEach(prop =>
            {
                var result = hashGenerator.GetForK(400, dictionary);
                var graphGenerator = new GraphGenerator();
                graphGenerator.AddSeries($"{graphName}{k}", SeriesChartType.Point, result.Select(x => (double)x.Key).ToList(), result.Select(x => x.Value / x.Key).ToList());
